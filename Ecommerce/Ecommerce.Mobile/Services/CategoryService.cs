...
namespace Ecommerce.Mobile.Services
{
    public class CategoryService
    {
        ...
        public async Task<List<Category>> GetCategoriesAsync()
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var categories = JsonSerializer.Deserialize<List<Category>>(json, _jsonSerializerOptions);
        ...
        public async Task<bool> UpdateCategoryAsync(Category category)
        {
            try
            {
                var endpoint = $"{_baseUrl}/categories/{category.Id}";
                var json = JsonSerializer.Serialize(category, _jsonSerializerOptions);
                var content = new StringContent(json, Encoding.UTF8, "application/json");
        ...
        public async Task<bool> DeleteCategoryAsync(int id)
        {
            try
            {
                var endpoint = $"{_baseUrl}/categories/{id}";
